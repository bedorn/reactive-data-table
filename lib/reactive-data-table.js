...
Template.reactiveDataTable.onRendered(function() {
    const placeholder = TAPi18n.__("table_search");
    this.$(".searchInputField input").attr("placeholder", placeholder).val("").focus().blur();
});
...
Template.reactiveDataTable.onCreated(function() {
    ...
    if (
        ...
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: collection argument is not an instance of Mongo.Collection or Cursor or Array"
        );
    }

    if (
        !_.isArray(Template.instance().data.fields) &&
        Template.instance().data.fields
    ) {
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: fields is not an instance of type array"
        );
    }
    ...
    if (
        ...
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: fields instance (array) is empty"
        );
    }

    if (
        !_.isString(Template.instance().data.tableClass) &&
        Template.instance().data.tableClass
    ) {
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: tableClass is not an instance of string"
        );
    }

    if (
        !_.isString(Template.instance().data.tableId) &&
        Template.instance().data.tableId
    ) {
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: tableId is not an instance of string"
        );
    }

    if (
        !_.isString(Template.instance().data.bodyClass) &&
        Template.instance().data.bodyClass
    ) {
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: bodyClass is not an instance of string"
        );
    }

    if (
        !_.isNumber(Template.instance().data.maxPageNumbers) &&
        Template.instance().data.maxPageNumbers
    ) {
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: maxPageNumbers is not an instance of type number"
        );
    } else if (
        Template.instance().data.maxPageNumbers &&
        ...
        throw new Meteor.Error(
            "bedorn:reactiveDataTable error: maxPageNumbers minimum is 5"
        );
    }
    ...
    const currentFirstPage = new ReactiveVar(
        Session.get("current-first-page") || 1
    );
...
Template.reactiveDataTable.events({
    ...
    "click .next-page"(event) {
        ...
        if (
            ...
            countCollection = collection.find().count();
        } else if (_.isArray(collection)) {
            countCollection = collection.length;
        }

        const countPages = parseInt(
            countCollection / Template.instance().rowsPerPage.get(),
            10
        );
...
Template.reactiveDataTable.helpers({
    ...
    sumEntries() {
        ...
        if (
            _.isBoolean(Template.instance().data.showOwnUser) &&
    ...
    isLastPageActive() {
        ...
        const currentPage = Template.instance().currentPage.get() + 1;
        const countPages = parseInt(
            _getDataCount(collection) / Template.instance().rowsPerPage.get(),
            10
        );
    ...
    pages() {
        ...
        let lastPageNumber =
            Template.instance().currentFirstPage.get() + maxPageNumbers;
    ...
    showSearchFilter() {
        if (
            !_.isBoolean(Template.instance().data.showSearchFilter) &&
            Template.instance().data.showSearchFilter
        ) {
            throw new Meteor.Error(
                "bedorn:reactiveDataTable error: showSearchFilter is not an instance of boolean"
            );
        }

        return _.isBoolean(Template.instance().data.showSearchFilter)
            ? Template.instance().data.showSearchFilter
            : true;
    },
    searchFilterId() {
        if (
            ...
            throw new Meteor.Error(
                "bedorn:reactiveDataTable error: searchFilterId is not an instance of string"
            );
        }
    ...
    showRowsPerPageFilter() {
        if (
            !_.isBoolean(Template.instance().data.showRowsPerPageFilter) &&
            Template.instance().data.showRowsPerPageFilter
        ) {
            throw new Meteor.Error(
                "bedorn:reactiveDataTable error: showRowsPerPage is not an instance of boolean"
            );
        }
    ...
    searchFields() {
        if (
            !_.isArray(Template.instance().data.searchFields) &&
            Template.instance().data.searchFields
        ) {
            throw new Meteor.Error(
                "bedorn:reactiveDataTable error: searchFields is not an instance of array"
            );
        }
        ...
        if (
            ...
            throw new Meteor.Error(
                "bedorn:reactiveDataTable error: searchFields instance (array) is empty"
            );
        }
