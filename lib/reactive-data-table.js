...
Template.reactiveDataTable.helpers({
    ...
    isLastPageActive() {
        const collection = Template.instance().data.collection;
        const currentPage = Template.instance().currentPage.get() + 1;
        const countPages = parseInt(collection.count() / Template.instance().rowsPerPage.get());
        const countPagesMod = collection.count() % Template.instance().rowsPerPage.get();
        let count = countPagesMod == 0 ? countPages : (countPages + 1);
    ...
    showPagination() {
        const collection = Template.instance().data.collection;
        return collection.count() > Template.instance().rowsPerPage.get();
    },
    pages() {
        const collection = Template.instance().data.collection;
        const countPages = parseInt(collection.count() / Template.instance().rowsPerPage.get());
