Template.reactiveDataTable.onCreated(function() {
    ...
    if (!(collection instanceof Mongo.Collection || collection instanceof Mongo.Collection.Cursor)) {
        throw new Meteor.Error("bedo:reactiveDataTable error: collection argument is not an instance of Mongo.Collection or Cursor");
    }
    if (!_.isArray(Template.instance().data.fields) && Template.instance().data.fields) {
        throw new Meteor.Error('bedo:reactiveDataTable error: fields is not an instance of type array');
    }
    if (Template.instance().data.fields && Template.instance().data.fields.length == 0) {
        throw new Meteor.Error('bedo:reactiveDataTable error: fields instance (array) is empty');
    }
    if (!_.isString(Template.instance().data.tableClass) && Template.instance().data.tableClass) {
        throw new Meteor.Error('bedo:reactiveDataTable error: tableClass is not an instance of string');
    }    
...
Template.reactiveDataTable.events({
    'change #clientRowsPerPage'(e, template) {
...
Template.reactiveDataTable.helpers({
    ...
    showSearchFilter() {
        if (!_.isBoolean(Template.instance().data.showSearchFilter) && Template.instance().data.showSearchFilter) {
            throw new Meteor.Error('bedo:reactiveDataTable error: showSearchFilter is not an instance of boolean');
        }
    ...
    showRowsPerPageFilter() {
        if (!_.isBoolean(Template.instance().data.showRowsPerPageFilter) && Template.instance().data.showRowsPerPageFilter) {
            throw new Meteor.Error('bedo:reactiveDataTable error: showRowsPerPage is not an instance of boolean');
        }
    ...
    searchFields() {
        if (!_.isArray(Template.instance().data.searchFields) && Template.instance().data.searchFields) {
            throw new Meteor.Error('bedo:reactiveDataTable error: searchFields is not an instance of array');
        }
        if(Template.instance().data.searchFields && Template.instance().data.searchFields.length == 0) {
            throw new Meteor.Error('bedo:reactiveDataTable error: searchFields instance (array) is empty');
        }
